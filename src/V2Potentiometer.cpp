...
void V2Potentiometer::measure(float analog) {
  ...
  if (_history.analog < config->min) {
    ...
    if (abs(now.fraction - _history.lag) < config->lag)
      return;

    now.step = roundf(now.fraction * (config->n_steps - 1));
  }

  _position.fraction = now.fraction;

  if (now.step == _position.step)
    return;

  _position.step = now.step;
  handleChange(_position.fraction, _position.step);
